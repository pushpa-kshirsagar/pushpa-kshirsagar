...
function* workerSignInAssesseeSaga(data) {
  try {
    ...
    if (userResponse.responseCode === '000') {
      ...
      if (
        userResponse?.responseObject?.signInResponse.isAssociateAssent === false &&
        userResponse?.responseObject?.signInResponse.isAssesseeAssent === false
      ) {
        yield put({ type: SET_SIGN_IN_STATUS, payload: 'confirmUSer' });
      } else {
